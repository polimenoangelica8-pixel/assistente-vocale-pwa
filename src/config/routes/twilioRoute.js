...
function buildStreamTwiml(host, params = {}) {
  const wsUrl = `wss://${host}/media-stream`;
  const callSid = xmlEscape(params.callSid);
  const from = xmlEscape(params.from);
...
function handleIncoming(req, res) {
  ...
  const twiml = buildStreamTwiml(host, { callSid, from });
